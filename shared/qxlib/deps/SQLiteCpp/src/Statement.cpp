...
#include "qxlib/db/QxDatabaseBenchmark.hpp"
#endif
...
void Statement::bind(const int aIndex, const int& aValue)
{
    ...
    check(ret, "Cannot bind int to index " + std::to_string(aIndex));
}
...
void Statement::bind(const int aIndex, const sqlite3_int64& aValue)
{
    ...
    check(ret, "Cannot bind int64 to index " + std::to_string(aIndex));
}
...
void Statement::bind(const int aIndex, const double& aValue)
{
    ...
    check(ret, "Cannot bind double to index " + std::to_string(aIndex));
}
...
void Statement::bind(const int aIndex, const std::string& aValue)
{
    ...
    check(ret, "Cannot bind std::string to index " + std::to_string(aIndex));
}
...
void Statement::bind(const int aIndex, const char* apValue)
{
    ...
    check(ret, "Cannot bind 'const char *' to index " + std::to_string(aIndex));
}
...
void Statement::bind(const int aIndex, const void* apValue, const int aSize)
{
    ...
    check(ret, "Cannot bind blob to index " + std::to_string(aIndex));
}
...
void Statement::bind(const int aIndex)
{
    ...
    check(ret, "Cannot bind NULL to index " + std::to_string(aIndex));
}
...
void Statement::bind(const char* apName, const int& aValue)
{
    ...
    check(ret, "Cannot bind int to param " + std::string(apName));
}
...
void Statement::bind(const char* apName, const sqlite3_int64& aValue)
{
    ...
    check(ret, "Cannot bind int64 to param " + std::string(apName));
}
...
void Statement::bind(const char* apName, const double& aValue)
{
    ...
    check(ret, "Cannot bind double to param " + std::string(apName));
}
...
void Statement::bind(const char* apName, const std::string& aValue)
{
    ...
    const int ret = sqlite3_bind_text(mStmtPtr, index, aValue.c_str(),
                                      static_cast<int>(aValue.size()), SQLITE_TRANSIENT);
    check(ret, "Cannot bind std::string to param " + std::string(apName));
}
...
void Statement::bind(const char* apName, const char* apValue)
{
    ...
    check(ret, "Cannot bind 'const char *' to param " + std::string(apName));
}
...
void Statement::bind(const char* apName, const void* apValue, const int aSize)
{
    ...
    check(ret, "Cannot bind blob to param " + std::string(apName));
}
...
void Statement::bind(const char* apName)
{
    ...
    check(ret, "Cannot bind NULL to param " + std::string(apName));
}
...
bool Statement::executeStep()
{
    ...
    {
#ifdef PROFILE_DATABASE_PERFORMANCE
        double startTime = qx::DatabaseBenchmark::time();
#endif
        const int ret = sqlite3_step(mStmtPtr);
#ifdef PROFILE_DATABASE_PERFORMANCE
        double endTime = qx::DatabaseBenchmark::time();
        qx::DatabaseBenchmark::registerInvocation(mQuery, endTime - startTime);
#endif
...
Column  Statement::getColumn(const char* apName)
{
    ...
    {
        throw SQLite::Exception("Unknown column name '" + std::string(apName) + "'");
    }
