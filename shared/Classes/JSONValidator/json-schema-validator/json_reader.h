...
class JSONReader {
 public:
  ...
  static base::Value* Read(const std::string& json, bool allow_trailing_comma);

  // Reads and parses |json| like Read(). |error_code_out| and |error_msg_out|
  // are optional. If specified and NULL is returned, they will be populated
  // an error code and a formatted error message (including error location if
  // appropriate). Otherwise, they will be unmodified.
  static base::Value* ReadAndReturnError(const std::string& json,
  ...
  // If |allow_trailing_comma| is true, we will ignore trailing commas in
  // objects and arrays even though this goes against the RFC.
  base::Value* JsonToValue(const std::string& json, bool check_root,
                     bool allow_trailing_comma);
 ...
 private:
  ...
  base::Value* BuildValue(bool is_root);
  ...
  base::Value* DecodeNumber(const Token& token);
  ...
  base::Value* DecodeString(const Token& token);
