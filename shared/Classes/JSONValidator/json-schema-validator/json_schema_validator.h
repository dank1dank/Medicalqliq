...
//
// The following features are partly implemented:
// - string.pattern - does only a string comparision (no regex)
...
class JSONSchemaValidator {
 ...
 private:
  ...
  void Validate(base::Value* instance, base::DictionaryValue* schema,
                const std::string& path, bool is_required);
  ...
  JSONSchemaValidator(const JSONSchemaValidator& other);
  JSONSchemaValidator& operator=(const JSONSchemaValidator& other);
};
