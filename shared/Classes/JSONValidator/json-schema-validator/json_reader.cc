...
//#if defined(OS_POSIX)
  return isfinite(number) != 0; //RA- changed to isfinite as per the forums
//#elif defined(OS_WIN)
    ...
    case Token::OBJECT_BEGIN:
      {
        ...
        while (token.type != Token::OBJECT_END) {
          ...
          std::string dict_key;
          dict_key_value->GetAsString(&dict_key);
    ...
    if ('\\' == c) {
      ...
      switch (c) {
        ...
        case 'x':
          ...
          break;
        case 'u':
              // Adam Sowa: Do not decode \uNNNN but keep in escaped form
          decoded_str.push_back('\\');
          decoded_str.push_back('u');
          decoded_str.push_back((*(token.begin + i + 1)));
          decoded_str.push_back((*(token.begin + i + 2)));
          decoded_str.push_back((*(token.begin + i + 3)));
          decoded_str.push_back((*(token.begin + i + 4)));
            i += 4;
            break;
