...
NSString * PrettyDateFromInterval( NSInteger epoch ) {
    ...
    //--------------------------------------------------
    // Maths
    NSInteger sec = epoch % 60;
    epoch -= sec;
    
    NSInteger minSeconds = epoch % 3600;
    epoch -= minSeconds;
    NSInteger min = (minSeconds / 60);
    
    NSInteger hourSeconds = epoch % 86400;
    epoch -= hourSeconds;
    NSInteger hour = (hourSeconds / 3600);
    
    NSInteger daySeconds = epoch % 604800;
    epoch -= daySeconds;
    NSInteger day = (daySeconds / 86400);
    
    //  int week = (epoch / 604800);
    
	
	NSInteger weekSeconds = epoch % 2419200;
	epoch -= weekSeconds;
    NSInteger week = (weekSeconds / 604800);
    
    //	int month = (epoch / 2419200);
	
	
	NSInteger monthSeconds = epoch % 29030400;
	epoch -= monthSeconds;
    NSInteger month = (monthSeconds / 2419200);
	
	NSInteger year = (epoch / 29030400);
	...
	if (year > 0) {
		singleOutput = [NSString stringWithFormat:@"%ld year%@" ,  (long)year , (
                                                                          year != 1 ? @"s" : @"")];
	}
	else if (month > 0) {
		singleOutput = [NSString stringWithFormat:@"%ld month%@" ,  (long)month , (
                                                                            month != 1 ? @"s" : @"")];
	}
    else if (week > 0) {
        singleOutput = [NSString stringWithFormat:@"%ld week%@" ,  (long)week , (
                                                                          week != 1 ? @"s" : @"")];
    }
    else if (day > 0) {
        //output[] = day . ' day' . (day != 1 ? 's' : '');
        singleOutput = [NSString stringWithFormat:@"%ld day%@" ,  (long)day , ( day
                                                                        != 1 ? @"s" : @"")];
    }
    else if (hour > 0) {
        //output[] = hour . ' hour' . (hour != 1 ? 's' : '');
        singleOutput = [NSString stringWithFormat:@"%ld hour%@" ,  (long)hour , (
                                                                          hour != 1 ? @"s" : @"")];
    }
    else if (min > 0) {
        //output[] = min . ' minute' . (min != 1 ? 's' : '');
        singleOutput = [NSString stringWithFormat:@"%ld min%@" ,  (long)min , ( min
                                                                        != 1 ? @"s" : @"")];
    }
    else {
        //output[] = sec . ' second' . (sec != 1 ? 's' : '');
        singleOutput = [NSString stringWithFormat:@"%ld sec%@" ,  (long)sec , ( sec
                                                                        != 1 ? @"s" : @"")];
    }
...
- (NSString*)prettyAge{
    ...
    unsigned int unitFlags = NSCalendarUnitMonth | NSCalendarUnitYear;
...
- (NSString*)durationInHoursAndMinutesSinceDate:(NSDate*)startDate{
    ...
    if([breakdownInfo minute] < 10){
        ...
        minutes = [NSString stringWithFormat:@"0%ld", (long)[breakdownInfo minute]];
    }else{
        
        minutes = [NSString stringWithFormat:@"%ld", (long)[breakdownInfo minute]];
    }
    
    return [NSString stringWithFormat:@"%ld:%@", (long)[breakdownInfo hour], minutes];
...
- (NSString*)relativeFormattedDate {
    ...
    for (i = -1; i < 7; i++) {
        ...
        if ([suppliedDate compare:referenceDate] == NSOrderedSame && i == 0) {
            ...
            // Yesterday
            [formatter setDateStyle:NSDateFormatterNoStyle];
            return [NSString stringWithString:LocalizedString(@"300-ValueYesterday")];
...
- (NSString*)relativeFormattedDateOnly {
    ...
    for (i = -1; i < 7; i++) {
        ...
        if ([suppliedDate compare:referenceDate] == NSOrderedSame && i == 0) {
            // Today
            [formatter setDateStyle:NSDateFormatterNoStyle];
            return [NSString stringWithString:LocalizedString(@"301-ValueToday")];
        } else if ([suppliedDate compare:referenceDate] == NSOrderedSame && i == 1) {
            // Yesterday
            [formatter setDateStyle:NSDateFormatterNoStyle];
            return [NSString stringWithString:LocalizedString(@"300-ValueYesterday")];
            ...
            // Yesterday
            [formatter setDateStyle:NSDateFormatterNoStyle];
            return [NSString stringWithString:LocalizedString(@"302-ValueTomorrow")];
        } else if ([suppliedDate compare:referenceDate] == NSOrderedSame) {
            // Day of the week
            return [[formatter weekdaySymbols] objectAtIndex:weekday];
        }
...
- (BOOL)isEarlierThanDate:(NSDate*)anotherDate{
	...
	if([[anotherDate earlierDate:self] isEqualToDate:self]) {
		return YES;
	} else {
		return NO;
	}	
