...
// Adam Sowa: MD5 comparison method which ignores leading zeros and spaces and is case insensitive
- (BOOL) isMd5Equal:(NSString *)other
{
    ...
    NSUInteger startIndex1 = 0;
    NSUInteger startIndex2 = 0;

    for (NSUInteger i = 0, len = [self length]; i < len; ++i) {
        if ([self characterAtIndex:i] == zeroChar || [self characterAtIndex:i] == spaceChar) {
            startIndex1++;
        } else {
            break;
        }
    ...
    for (NSUInteger i = 0, len = [other length]; i < len; ++i) {
        if ([other characterAtIndex:i] == zeroChar || [other characterAtIndex:i] == spaceChar) {
            startIndex2++;
        } else {
            break;
        }
    ...
    // After skiping zero and space both md5 must have equal length left for comparison
    if (([self length] - startIndex1) != ([other length] - startIndex2)) {
        return false;
    }

    NSUInteger i1 = startIndex1;
    NSUInteger i2 = startIndex2;
    for (const NSUInteger firstSize = [self length]; i1 < firstSize; ++i1, ++i2) {
        if (tolower([self characterAtIndex:i1]) != tolower([other characterAtIndex:i2])) {
            return false;
        }
    }
...
- (void)removeLastCharacter{
    ...
    [self deleteCharactersInRange:NSMakeRange((self.length > 0 ? self.length-1 : 0), 1)];
}
