...
#pragma clang diagnostic ignored "-Wint-to-pointer-cast"
        port = [NSString stringWithFormat: @"%s", (char *) ntohl(addrIn.sin_port)];
...
- (void) privateShowAlert: (id) formatstring,...
{
    ...
    UIAlertView_Blocks *av = [[UIAlertView_Blocks alloc] initWithTitle:outstring
                                                               message:nil
                                                              delegate:nil
                                                     cancelButtonTitle:NSLocalizedString(@"1-ButtonOK", nil)
                                                     otherButtonTitles:nil];
	[av showWithDissmissBlock:NULL];
}
...
- (BOOL) performWiFiCheck
{
	...
	{
		[self performSelector:@selector(privateShowAlert:) withObject:NSLocalizedString(@"1207-TextThisApplicationRequiresWiFi", nil) afterDelay:0.5f];
		return NO;
	}
	return YES;
}
...
- (BOOL) forceWWAN
{
	...
	DDLogSupport(@"%@", myInfoPtr ? @"Started WWAN" : @"Failed to start WWAN");
...
- (BOOL) scheduleReachabilityWatcher: (id) watcher
{
	if (![watcher respondsToSelector:@selector(reachabilityChanged)]) 
	{
		DDLogSupport(@"Watcher must implement reachabilityChanged callback. Cannot continue.");
		return NO;
	}
	...
	if(SCNetworkReachabilitySetCallback(reachability, ReachabilityCallback, &context)) 
	{
		...
		{
			DDLogSupport(@"Error: Could not schedule reachability");
	...
	{
		DDLogSupport(@"Error: Could not set reachability callback");
		return NO;
	}
...
- (void) unscheduleReachabilityWatcher
{
	...
	if (SCNetworkReachabilityUnscheduleFromRunLoop(reachability, CFRunLoopGetCurrent(), kCFRunLoopCommonModes))
		DDLogSupport(@"Unscheduled reachability");
	else
		DDLogSupport(@"Error: Could not unschedule reachability");
