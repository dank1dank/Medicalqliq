...
- (void)setup {
    ...
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0.f, 0.f, self.frame.size.width, self.frame.size.height)];
...
- (void)addOneContact:(id)contact withName:(NSString *)name {
    id contactKey = [NSValue valueWithNonretainedObject:contact];
    if ([self.contactKeys containsObject:contactKey]){
        NSLog(@"Cannot add the same object twice to ContactPickerView");
        return;
    ...
    THContactBubble *contactBubble = [[THContactBubble alloc] initWithName:name style:self.bubbleStyle selectedStyle:self.bubbleSelectedStyle showComma:NO];
    contactBubble.keyboardAppearance = self.keyboardAppearance;
    if (self.font != nil){
        [contactBubble setFont:self.font];
    }
...
- (BOOL)isContactPickerFirstResponder {
    return [self.textView isFirstResponder] || [self.selectedContactBubble isFirstResponder] || [self.scrollView isFirstResponder] || [self isFirstResponder] || [self.selectedContactBubble.textView isFirstResponder];
}
...
- (void)layoutView {
     ...
     */
    {
        if (self.delegate && [self.delegate respondsToSelector:@selector(getContactPickerWidth)])
        {
            CGFloat contactPickerWidth = [self.delegate getContactPickerWidth];
            self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, contactPickerWidth, self.frame.size.height);
        }
    }
...
- (void)textFieldDidBeginEditing:(THContactTextField *)textField {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(contactPickerDidBeginEditing:)]) {
